...
def main():
    ...
    # Load pre-committed dataset (no download needed)
    ...
    print(" Model evaluation completed (1000 trials)")
    ...
    # Save test predictions to model/ directory (for app)
    pd.DataFrame({'y_test': y_test, 'y_prob': y_prob}).to_csv(
        'model/prediction_data.csv', index=False
    )
    print(" Test predictions saved to model/prediction_data.csv")
